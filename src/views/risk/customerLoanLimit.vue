<template>
  <div>
    ...
    <el-card id="search">
      <el-row>
        <el-col :span="20" class="search-bar custom-search-bar">
          ...
          ></el-input>
          <el-select
            v-model="searchModel.loanLimitLevel"
            clearable
            placeholder="贷款额度等级"
          >
            <el-option
              v-for="item in loanLimitLevelList"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            >
          ...
          <el-select
            v-model="searchModel.evaluateStatus"
            clearable
            placeholder="评估状态"
          >
            <el-option
              v-for="item in evaluateStatusList"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            >
          ...
          <el-date-picker
            v-model="searchModel.evaluateDate"
            type="daterange"
            align="right"
            unlink-panels
            range-separator="至"
          ...
          <el-button
            @click="getCustomerLoanLimitList"
            type="primary"
            round
            icon="el-icon-search"
            >查询</el-button
    ...
    <el-card>
      <el-table :data="customerLoanLimitList" stripe style="width: 100%">
        ...
        <el-table-column prop="loanLimitLevel" label="贷款额度等级" width="80">
          <template slot-scope="scope">
            <el-tag v-if="scope.row.loanLimitLevel == 'S'" type="success"
              >S类客户</el-tag
            >
            <el-tag v-if="scope.row.loanLimitLevel == 'A+'" type="success"
              >A+类客户</el-tag
            >
            <el-tag v-if="scope.row.loanLimitLevel == 'A'" type="success"
              >A类客户</el-tag
            ...
            <el-tag v-if="scope.row.loanLimitLevel == 'A-'" type="success"
              >A-类客户</el-tag
            >
            <el-tag v-if="scope.row.loanLimitLevel == 'B'" type="success"
              >B类客户</el-tag
            >
            <el-tag v-if="scope.row.loanLimitLevel == 'C+'" type="success"
              >C+类客户</el-tag
            >
            <el-tag v-if="scope.row.loanLimitLevel == 'C'" type="success"
              >C类客户</el-tag
            >
            <el-tag v-if="scope.row.loanLimitLevel == 'D'" type="success"
              >D类客户</el-tag
            >
            <el-tag v-if="scope.row.loanLimitLevel == 'E'" type="success"
              >E类客户</el-tag
            >
            <el-tag v-if="scope.row.loanLimitLevel == 'F+'" type="success"
              >F+类客户</el-tag
            >
            <el-tag v-if="scope.row.loanLimitLevel == 'F'" type="success"
              >F类客户</el-tag
            >
            <el-tag v-if="scope.row.loanLimitLevel == 'G'" type="danger"
              >G类客户</el-tag
            >
          </template>
        </el-table-column>
        <el-table-column prop="evaluateStatus" label="评估状态" width="80">
          <template slot-scope="scope">
            <el-tag v-if="scope.row.evaluateStatus == '1'" type="warning"
              >待评估</el-tag
            >
            <el-tag v-if="scope.row.evaluateStatus == '2'" type="success"
              >已评估</el-tag
            >
          </template>
        </el-table-column>
    ...
    >
      ...
      >
        <el-form-item
          v-if="evaluateForm.id == null || evaluateForm.id == undefined"
          label="身份证号码"
          prop="idCard"
          :label-width="formLabelWidth"
        >
          <el-input
            v-model="evaluateForm.idCard"
            autocomplete="off"
            @blur="handleBlur"
            @input="handleInput"
          ></el-input>
        ...
        >
          <el-input
            v-model="evaluateForm.customerName"
            autocomplete="off"
            :disabled="true"
          ></el-input>
        </el-form-item>
        ...
        >
          <el-input
            v-model="evaluateForm.customerPhone"
            autocomplete="off"
            :disabled="true"
          ></el-input>
        </el-form-item>
        ...
        >
          <el-input
            v-model="evaluateForm.occupation"
            autocomplete="off"
            :disabled="isEvaluate"
          ></el-input>
        </el-form-item>
        ...
        >
          <el-input
            v-model="evaluateForm.annualIncome"
            autocomplete="off"
        ...
        >
          <el-input
            v-model="evaluateForm.careerYears"
            autocomplete="off"
            :disabled="isEvaluate"
          ></el-input>
        </el-form-item>
        ...
        >
          <el-input
            v-model="evaluateForm.debtAmount"
            autocomplete="off"
            @blur="handleBlurAssetCalculate"
            @input="handleInputAssetCalculate"
            :disabled="isEvaluate"
          ></el-input>
        </el-form-item>
        <el-form-item
        ...
        >
          <el-input
            v-model="evaluateForm.houseProperty"
            autocomplete="off"
            @blur="handleBlurAssetCalculate"
            @input="handleInputAssetCalculate"
            :disabled="isEvaluate"
        ...
        >
          <el-input
            v-model="evaluateForm.carProperty"
            autocomplete="off"
            @blur="handleBlurAssetCalculate"
            @input="handleInputAssetCalculate"
            :disabled="isEvaluate"
        ...
        >
          <el-input
            v-model="evaluateForm.monthDebtRepay"
            autocomplete="off"
            @blur="handleBlurCalculate"
            @input="handleInputCalculate"
            :disabled="isEvaluate"
        ...
        >
          <el-input
            v-model="evaluateForm.assetValuation"
            autocomplete="off"
            :disabled="true"
          ></el-input>
        </el-form-item>
        ...
        >
          <el-input
            v-model="evaluateForm.netAssetValue"
            autocomplete="off"
            :disabled="true"
          ></el-input>
        </el-form-item>
        ...
        >
          <el-input
            v-model="evaluateForm.creditLevel"
            autocomplete="off"
            :disabled="true"
          ></el-input>
        </el-form-item>
        <el-form-item
          v-if="op == 1"
          label="当前贷款额度等级"
          prop="oldLoanLimitLevel"
          :label-width="formLabelWidth"
        >
          <el-input
            v-model="evaluateForm.oldLoanLimitLevel"
            autocomplete="off"
            :disabled="true"
          ></el-input>
        </el-form-item>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="operateCustomerLoanLimit()"
          >确 定</el-button
        >
      </div>
    </el-dialog>
...
import customerCreditApi from "@/api/customerCreditManage";
...
export default {
  data() {
    ...
    return {
      ...
      //贷款额度等级(S(100亿元)、A+(10亿元)、A(1亿元)、A-(1千万元)、B(1百万元)、C+(50万元)、C(10万元)、D(1万元)、E(5千元)、F+(3千元)、F(2千元)、G(0元))
      loanLimitLevelList: [
        {
          value: "S",
          label: "S类客户(100亿元)",
  ...
  methods: {
    calculateDTI() {
      const { monthDebtRepay, annualIncome } = this.evaluateForm;
    ...
    calculateAsset() {
      ...
      this.evaluateForm.assetValuation =
        Number(houseProperty) +
        Number(carProperty) +
        Number(deposit) +
        Number(invest);
      this.evaluateForm.netAssetValue =
        this.evaluateForm.assetValuation - debtAmount;
      this.$forceUpdate();
    },
      ...
      this.$refs.evaluateFormRef.validate((valid) => {
        ...
        if (valid) {
          ...
          customerLoanLimitApi
            .operateCustomerLoanLimit(this.evaluateForm)
            .then((response) => {
    ...
    async openEditUI(row, op) {
      this.op = op; // 0：编辑；1：评估
      if (row == null) {
        ...
        try {
          // 根据id查询用户数据
          const response = await customerLoanLimitApi.getLoanLimitById({
            id: row.id,
          });
          this.evaluateForm = response.data;
          if (op) {
            const loanLimitResponse =
              await customerLoanLimitApi.getLoanLimitByCustomer({
                customerId: row.customerId,
              });
            this.evaluateForm.oldLoanLimitLevel =
              loanLimitResponse.data.loanLimitLevelName;

            const creditLevelResponse = await customerCreditApi.getCreditLevel({
              customerId: row.customerId,
            });
            const value = creditLevelResponse.data.creditLevel;
            const item = this.creditLevelList.find(
              (obj) => obj.value === value
            );
            this.evaluateForm.creditLevel = item ? item.label : "";
          }
        } catch (error) {
          console.error(error);
        }
    ...
    getCustomerLoanLimitList() {
      customerLoanLimitApi
        .getCustomerLoanLimitList(this.searchModel)
        .then((response) => {
    ...
    getCustomerByIdCard() {
      customerApi
        .getCustomerByIdCard(this.evaluateForm.idCard)
        .then((response) => {
